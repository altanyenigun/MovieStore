...
{
    public class CreateMovieValidatorTest : IClassFixture<CommonTestFixture>
    {
        private readonly FakeDataContext _context;
        private readonly IMapper _mapper;

        public CreateMovieValidatorTest(CommonTestFixture testFixture)
        {
        ...
        [Fact]
        public async void Validate_UniqueMovieName()
        {
            // Arrange
            var validator = new CreateMovieValidator(_context);
            ...
            var movieDto = new MovieCreateRequest { Name = randomMovie.Name, Year = 2000, Price = 10.0M, GenreIds = new List<int> { 1 }, DirectorId = 1, ActorIds = new List<int> { 1, 2 } };
            ...
            // Assert
            Assert.False(result.IsValid);
            Assert.Equal("Movie Name already registered.", result.Errors.First().ErrorMessage);
        }
